...
from train import train_epoch, train_epoch_softlabel, validate
...
def load_data_path(args):
    ...
    if args.dataset == 'imagenet':
        ...
        data, target = torch.load(os.path.join(f'{args.syn_data_dir}', 'data_best.pt'))
        print("Load condensed data ", args.syn_data_dir, data.shape)
...
def evaluate_syn_data(args, train_loader, val_loader, logger=None):
    ...
    teacher_path = os.path.join(args.pretrain_dir, f"{args.dataset}-{args.net_type}-ipc{args.ipc}.pkl")
    local_weights = torch.load(teacher_path)
    teacher_model.load_state_dict(local_weights)
    train_criterion_sl = SoftCrossEntropy

    train_criterion = nn.CrossEntropyLoss().cuda()
    val_criterion = nn.CrossEntropyLoss().cuda()
    model = copy.deepcopy(teacher_model) if teacher_model is not None else convnet3(args, args.nclass)
    teacher_model.eval()
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.softlabel and epoch < args.epochs / 2:
            acc1_tr, acc5_tr, loss_tr = train_epoch_softlabel( #这个softlabel为什么会导致性能下降得这么厉害，这很不对劲，需要仔细检查
                args,
                train_loader,
                model,
                teacher_model,
        ...
        pbar.set_description(
            f"[Epoch {epoch}/{args.epochs}] (Train) Top1 {acc1_tr:.1f}  Top5 {acc5_tr:.1f} Lr {optimizer.param_groups[0]['lr']} Loss {loss_tr:.3f}"
        )
        pbar.update(1)
        ...
        if (
                epoch % args.epoch_print_freq == 0
                or epoch == args.epochs
                or (epoch % (args.epoch_print_freq / 50) == 0 and args.ipc > 50)
        ):
            ...
            print("-------Eval Training Epoch [{} / {}] INFO--------".format(epoch, args.epochs))
            print(f"Current accuracy (top-1 and 5): {acc1:.1f} {acc5:.1f}, loss: {loss_val:.3f}")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--width", type=str, default=1.0)
    parser.add_argument("--depth", type=str, default=3)
    parser.add_argument("--ipc", type=str, default=10)
    ...
    parser.add_argument("--epoch_print_freq", type=int, default=100)
    ...
    wandb.init(
        ...
        project=f'Manifold_{args.dataset}_softlabel_test',
        name=f'Test softlabel with hyperbolic',
        mode=mode,
    )
    ...
    evaluate_syn_data(args, train_loader, val_loader)
