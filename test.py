        ...
        if name == '':
            if args.dataset == 'imagenet':
                if args.nclass == 10:
                    ...
                    name = f'imagenet_100/kernel{args.kernel}_ipc{args.ipc}_factor{args.factor}_il{args.il}'
            else:
                # name = f'{args.dataset}/kernel{args.kernel_type}_ipc{args.ipc}_factor{args.factor}_il{args.il}'
                name = f'{args.dataset}/ipc{args.ipc}'
        ...
        if 'idc' in args.slct_type:
            if args.slct_type == 'idc':
                ...
                for idx in range(args.nclass // args.nclass_sub):
                    path = f'{args.save_dir}_phase{idx}'
